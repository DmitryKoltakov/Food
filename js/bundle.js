...
// получаем функцию из сервисов в которые мы ее вырезали
...
//вызываем в одном модуле кусочек другого модуля
...
//поэтому оригинальную функцию нужно подправить чтобы она получала как аргумент селекторы и передавала их в переменную modal
 ...
 // функцию пост дата мы вынесли в services и тут ее принимаем
...
window.addEventListener('DOMContentLoaded', function() {
    const modalTimerId = setTimeout( () => Object(_modules_modal__WEBPACK_IMPORTED_MODULE_1__["openModal"])('modal', modalTimerId), 300000);// тут стрелочная функция сделана чтобы можно было передать аргументы в опен модал а не просто написать openModal без скобочек.Наебываем сет таймаут как с обработчиком событий
...
//вызываем каждую функцию
    ...
    Object(_modules_modal__WEBPACK_IMPORTED_MODULE_1__["default"])('[data-modal]','.modal', modalTimerId);// тут через аргументы делаем чтобы избежать багов
    Object(_modules_timer__WEBPACK_IMPORTED_MODULE_2__["default"])('.timer', "2020-10-04");
    ...
    Object(_modules_slider__WEBPACK_IMPORTED_MODULE_6__["default"])({// тут передаем все аргументы слайдера через деструктуризацию
